...
export const LandingPage: React.FC = () => {
  ...
  return (
    <div className="space-y-16 pb-20 animate-fade-in">
      {/* HERO SECTION - Spacious Clean Light Pastel Canvas */}
      <section className="relative overflow-hidden pt-12 pb-20 px-4 sm:px-6 lg:px-8 border-b border-slate-200/80 bg-gradient-to-br from-pink-50/70 via-sky-50/50 to-emerald-50/60">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          
          {/* Left Column Text & CTAs */}
          <div className="lg:col-span-7 space-y-6">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-rose-50 border border-rose-200 text-rose-700 text-xs font-bold shadow-2xs">
              <span className="w-2 h-2 rounded-full bg-rose-500 animate-ping"></span>
              24x7 STATE DISASTER RESPONSE PLATFORM FOR ASSAM
            </div>

            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-heading font-extrabold tracking-tight text-slate-900 leading-[1.12]">
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              <button
                ...
                className="bg-gradient-to-r from-rose-500 to-pink-600 hover:from-rose-600 hover:to-pink-700 text-white font-extrabold px-6 py-3.5 rounded-2xl shadow-md shadow-pink-500/20 border border-rose-300 flex items-center gap-2.5 text-sm animate-sos-pulse hover:scale-[1.02] transition-all"
              >
              ...
              <button
                ...
                className="bg-white hover:bg-emerald-50 text-emerald-800 font-bold px-5 py-3.5 rounded-2xl border border-slate-200 hover:border-emerald-300 shadow-2xs flex items-center gap-2 text-sm transition-all"
              >
              ...
              <button
                ...
                className="bg-white hover:bg-purple-50 text-purple-800 font-bold px-5 py-3.5 rounded-2xl border border-slate-200 hover:border-purple-300 shadow-2xs flex items-center gap-2 text-sm transition-all"
              >
            ...
            <div className="pt-6 border-t border-slate-200/80 grid grid-cols-3 gap-4 text-xs text-slate-600 font-semibold">
          ...
          {/* Right Column - Sleek Visual Live Stream */}
          <div className="lg:col-span-5">
            <div className="card-clean p-6 space-y-5 bg-white border border-slate-200/90 shadow-lg relative">
              ...
              <div className="space-y-3">
                {incidents.slice(0, 3).map(inc => (
                  <div key={inc.id} className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200/80 hover:border-pink-300 transition-all duration-200">
      ...
      {/* LIVE STATS COUNTER GRID - Clean White Cards with Hover Lift */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[
            ...
            return (
              <div key={i} className={`card-clean p-5 space-y-2 border border-slate-200 bg-white ${stat.bg}`}>
                <div className="flex items-center justify-between">
                  <Icon className={`w-5 h-5 ${stat.color}`} />
                  <span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded-full text-slate-600 font-bold border border-slate-200">{stat.badge}</span>
                </div>
                <div className={`text-2xl sm:text-3xl font-heading font-extrabold ${stat.color}`}>
                  {stat.count}
                </div>
                <div className="text-xs text-slate-600 font-bold">{stat.label}</div>
              </div>
            );
          })}
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[
            ...
            <div key={item.step} className="card-clean p-6 bg-white space-y-3">
              <span className={`text-xs font-extrabold px-2.5 py-1 rounded-lg bg-gradient-to-r ${item.color} text-white shadow-2xs`}>
                STEP {item.step}
              </span>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="space-y-3">
          ...
          <div className="space-y-3">
            <div className="w-10 h-10 rounded-2xl bg-pink-50 border border-pink-200 flex items-center justify-center text-pink-700">
