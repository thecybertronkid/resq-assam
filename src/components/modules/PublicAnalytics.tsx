    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      <div className="bg-gradient-to-r from-sky-100 via-pink-50 to-emerald-100 border border-sky-200 p-6 rounded-3xl flex flex-wrap items-center justify-between gap-4 shadow-sm">
        <div>
          ...
          <h1 className="text-2xl font-heading font-extrabold text-slate-900">Assam Emergency Operational Metrics</h1>
          <p className="text-xs text-slate-600 font-medium">Open government telemetry data for citizens, press, and international relief auditors.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-3xl border border-emerald-200 shadow-sm space-y-1">
          <span className="text-xs text-slate-500 font-bold block">Total Evacuated & Rescued</span>
          <div className="text-3xl font-heading font-extrabold text-emerald-700">4,289 Persons</div>
          <span className="text-[11px] text-emerald-700 font-bold">✓ 100% Verified Log</span>
        </div>

        <div className="bg-white p-5 rounded-3xl border border-sky-200 shadow-sm space-y-1">
          <span className="text-xs text-slate-500 font-bold block">Avg NDRF Motorboat Arrival</span>
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {ASSAM_DISTRICTS.slice(0, 8).map(dist => {
            ...
            return (
              <div key={dist} className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
                <div className="flex justify-between items-center text-xs font-bold text-slate-900">
                  <span>{dist}</span>
                  <span className="text-rose-700 bg-rose-100 px-2 py-0.5 rounded font-bold border border-rose-200">{incCount} Incidents</span>
                </div>
                <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                  <div className="bg-rose-500 h-full" style={{ width: `${loadPct}%` }} />
                </div>
              </div>
            );
          })}
